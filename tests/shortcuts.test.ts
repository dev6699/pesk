...
test("matches exact shortcut modifiers", () => {
  ...
  expect(
    matchesShortcut(event("Enter", { altKey: true, shiftKey: true }), "steer"),
  ).toBe(false);
});

test("supports Ctrl or Cmd shortcuts", () => {
  expect(matchesShortcut(event("c", { ctrlKey: true }), "copyMessage")).toBe(
    true,
  );
  expect(matchesShortcut(event("c", { metaKey: true }), "copyMessage")).toBe(
    true,
  );
  expect(matchesShortcut(event("c", { altKey: true }), "copyMessage")).toBe(
    false,
  );
});
...
test.each(Object.keys(SHORTCUTS) as ShortcutId[])(
  "matches the registered definition: %s",
  (id) => {
